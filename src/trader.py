...
class TradeExecutor:
    ...
    def execute_decision(self, decision: TradeDecision) -> Optional[Order]:
        ...
        log.info(f"  Signal: {decision.signal.value}, Confidence: {decision.confidence:.0%}")
    ...
    def _record_trade(self, decision: TradeDecision, order: Optional[Order],
    ...
    def get_trade_stats(self) -> Dict:
        ...
        buy_trades = sum(1 for t in self.trade_history 
                         if t['action'] in ['open_long', 'close_short'])
        sell_trades = sum(1 for t in self.trade_history 
                          if t['action'] in ['open_short', 'close_long'])
