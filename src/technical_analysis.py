...
Enhanced with:
- Stricter RSI thresholds (30/70 for proper overbought/oversold)
- Momentum confirmation requirements
- Volume-weighted signal strength
- Multi-indicator agreement filters
"""
...
class TechnicalAnalysisResult:
    ...
    def __str__(self):
        return (
            f"TA Result for {self.symbol}: {self.combined_signal.value} "
...
class TechnicalAnalyzer:
    ...
    Enhanced Features:
    - Stricter default thresholds for higher quality signals
    - Momentum confirmation before signal generation
    - Configurable minimum indicator agreement
    """

    # Strict RSI thresholds for better signal quality
    # Default config may use looser thresholds for more trades
    # These are used as fallback for high-quality mode
    STRICT_RSI_OVERSOLD = 30
    STRICT_RSI_OVERBOUGHT = 70
    ...
    def __init__(self, strict_mode: bool = False):
        ...
        Args:
            strict_mode: If True, use stricter thresholds for higher quality signals
        """
        self.config = settings.trading
        self.strict_mode = strict_mode

        # Use stricter thresholds if enabled
        if strict_mode:
            self._rsi_oversold = self.STRICT_RSI_OVERSOLD
            self._rsi_overbought = self.STRICT_RSI_OVERBOUGHT
            self._min_agreement = self.MIN_INDICATORS_NORMAL_SIGNAL
        else:
            self._rsi_oversold = self.config.rsi_oversold
            self._rsi_overbought = self.config.rsi_overbought
            self._min_agreement = self.config.min_signal_agreement
    ...
    def analyze(
        ...
        if len(close) < 50:
            log.warning(
                f"Insufficient data for TA: {len(close)} candles, need at least 50"
            )
        ...
        if result.combined_signal != Signal.HOLD:
            log.info(f"TA SIGNAL for {symbol}: {result.combined_signal.value} | Strength: {result.signal_strength} | Confidence: {result.confidence:.2f} | Bullish: {bullish_count} | Bearish: {bearish_count} | ADX: {adx if 'adx' in locals() else 'N/A'}")
        else:
    ...
    def _calculate_rsi(self, close: np.ndarray) -> IndicatorResult:
        ...
        Uses instance thresholds (strict_mode or config-based):
        RSI < oversold threshold = Oversold (Bullish)
        RSI > overbought threshold = Overbought (Bearish)
        ...
        # Get RSI momentum (is it rising or falling?)
        rsi_momentum = 0
        if len(rsi) >= 3:
            prev_rsi = rsi[-3] if not np.isnan(rsi[-3]) else current_rsi
            rsi_momentum = current_rsi - prev_rsi

        # Use instance thresholds (stricter or config-based)
    ...
    def _calculate_macd(self, close: np.ndarray) -> IndicatorResult:
        ...
        return IndicatorResult(
            name="MACD", signal=signal_type, value=current_hist, description=description
        )
    ...
    def _calculate_bollinger_bands(self, close: np.ndarray) -> IndicatorResult:
        ...
        return IndicatorResult(
            name="Bollinger Bands", signal=signal, value=pct_b, description=description
        )
    ...
    def _calculate_ema_crossover(self, close: np.ndarray) -> IndicatorResult:
        ...
        return IndicatorResult(
            name="EMA Crossover", signal=signal, value=diff, description=description
        )
    ...
    def _generate_combined_signal(
        ...
        # If there are conflicting signals (both bullish and bearish), be more conservative
        ...
        strong_threshold = (
            total_indicators - 1 if total_indicators >= 4 else total_indicators
        )
    ...
    def should_close_long(self, result: TechnicalAnalysisResult) -> bool:
        ...
        # Closing is more lenient - we don't want to hold losing positions
    ...
    def should_open_long(self, result: TechnicalAnalysisResult) -> bool:
        """Alias for should_enter_long for compatibility."""
        return self.should_enter_long(result)

    def should_open_short(self, result: TechnicalAnalysisResult) -> bool:
        """Alias for should_enter_short for compatibility."""
        return self.should_enter_short(result)
    ...
    def calculate_atr_stop(
        self, entry_price: float, atr: float, side: str, multiplier: float = 2.0
    ) -> float:
        """
    ...
    def get_trend_direction(
        self, close: np.ndarray, ema_short: int = 50, ema_long: int = 200
    ) -> str:
        """
    ...
    def analyze_with_volume(
        ...
        result.combined_signal, result.signal_strength = self._generate_combined_signal(
            result.indicators
        )
...
class MultiTimeframeAnalyzer:
    ...
    def __init__(self, client, higher_tf: str = "4h", entry_tf: str = "15m"):
        """
    ...
    def analyze(self, symbol: str) -> dict:
        ...
        try:
            ...
            entry_candles = self.client.get_candles(
                symbol=symbol, resolution=self.entry_tf
            )
            ...
            should_trade = (
                is_aligned or htf_trend == "neutral"
            ) and entry_result.confidence >= 0.25

            # DEBUG: Log detailed analysis
            log.info(f"[MTF] {symbol} Analysis:")
            log.info(f"  Higher TF ({self.higher_tf}) Trend: {htf_trend.upper()}")
            log.info(
                f"  Entry TF ({self.entry_tf}) Signal: {entry_result.combined_signal.value}"
            )
            ...
            if not should_trade:
                if not is_aligned and htf_trend != "neutral":
                    log.info(
                        f"  → NO TRADE: Trend ({htf_trend}) conflicts with entry signal ({entry_result.combined_signal.value})"
                    )
                elif entry_result.confidence < 0.25:
                    log.info(
                        f"  → NO TRADE: Confidence {entry_result.confidence:.2f} < 0.25 threshold"
                    )
            else:
                if htf_trend == "neutral":
                    log.info(
                        f"  → READY TO TRADE: {entry_result.combined_signal.value} signal (trend is neutral)"
                    )
                else:
                    log.info(
                        f"  → READY TO TRADE: {htf_trend.upper()} trend + {entry_result.combined_signal.value} entry"
                    )
    ...
    def _check_alignment(
        self, htf_trend: str, entry_result: TechnicalAnalysisResult
    ) -> bool:
        """
    ...
    def should_enter_long(self, symbol: str) -> tuple:
        ...
        if analysis["higher_tf_trend"] == "bullish" and analysis["entry_signal"] in [
            "buy",
            "strong_buy",
        ]:
    ...
    def should_enter_short(self, symbol: str) -> tuple:
        ...
        if analysis["higher_tf_trend"] == "bearish" and analysis["entry_signal"] in [
            "sell",
            "strong_sell",
        ]:
            return True, analysis["entry_confidence"], analysis["atr"]
